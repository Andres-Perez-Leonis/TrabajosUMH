...
int main(int argc,char *argv[])
{
    ...
    */
    ...
    for(int i = 0; i < nprocess; i++) {
        ...
        if(i != 0) comienzo_proceso[i] = comienzo_proceso[i-1] + comp_proceso;
    }
    ...
    if(myrank == 0) {
        ...
        obtener_valores(matriz_M, tam_N, tam_N, "");

        

        for(int i = 0; i < 3; i++){
            for(int j = 0; j < 3; j++){
                printf("Valor pos[%d][%d]: %lf\n", i, j, matriz_M[i][j]);
            }
        }
        ...
        for(int i = 1; i < nprocess; i++)
            for(int j = comienzo_proceso[i]; j < comienzo_proceso[i] + n_comp_proceso[i]; j++)
                MPI_Send(matriz_M[i], tam_N, MPI_DOUBLE, i, 1, MPI_COMM_WORLD);

        
        for(int i = 0; i < tam_N; i++){
            matriz_vect_iter[0][i] = 1.0;
        }
        //printf("EL VALOR DE LOS COMPONENTES QUE ME TOCAN SON: %d\n", n_comp_proceso[myrank]);
        ...
        for(int i = 1; i < nIteraciones; i++){
            ...
            for(int j = 0; j < n_comp_proceso[myrank]; j++) {
                ...
                printf("VALOR PRODUCTO: %llf\n", prod_sum);
                // if(i != 1) {
                //     matriz_vect_iter[i][j] = matriz_vect_iter[i-1][j] - prod_sum;
                // }
                // sum += prod_sum;
            }
            // MPI_Allreduce(&sum, &sum_general, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
            // printf("Valor: %llf\n", sum_general);
            // normalizar_vector(matriz_vect_iter[i], tam_N, sum_general);
        }
        ...
        matriz_vect_iter = reserva_memoria(matriz_vect_iter, tam_N, nIteraciones); //Posiblemente necesite cambios
        ...
        for(int i = 1; i < nIteraciones; i++){
            ...
            printf("ALGO\n");
            //sum = obtener_sum_cuadrado(matriz_vect_iter[i], tam_N);
            MPI_Allreduce(&sum, &sum_general, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
            normalizar_vector(matriz_vect_iter[i], tam_N, sum_general);
        }
...
double obtener_Multiplicacion(double **matriz_1, double **matriz_2, int tam, int pos_i, int pos_j){
    double aux = 0;
    for(int k = 0; k < tam; k++){
        printf("POSICION MATRIZ 1 [%d][%d] VALOR: %llf\n", pos_i, k, matriz_1[pos_i][k]);
        printf("POSICION MATRIZ 2 [%d][%d] VALOR: %llf\n", pos_i, k, matriz_2[pos_j][k]);
